function [ F_add ] = F_adjustment_Hu(Ak, coordinatesMatrix,Vmu,Kmu,Kmv,Cuamb,Cvamb)
    ...
    %Worked out dubbele integral
     Fe1 = ((2*Ak)/24)*(2*x1 + x2 + x3)*((Vmu*Cuamb)/((Kmu+Cuamb)*(1+Cvamb/Kmv)))*(1-(Kmu/(Kmu+Cuamb))+(Cvamb/(Kmv*(1+Cvamb/Kmv))));
     Fe2 = ((2*Ak)/24)*(x1 + 2*x2 + x3)*(Vmu*Cuamb)/((Kmu+Cuamb)*(1+Cvamb/Kmv))*(1-(Kmu/(Kmu+Cuamb))+(Cvamb/(Kmv*(1+Cvamb/Kmv))));
     Fe3 = ((2*Ak)/24)*(x1 + x2 + 2*x3)*(Vmu*Cuamb)/((Kmu+Cuamb)*(1+Cvamb/Kmv))*(1-(Kmu/(Kmu+Cuamb))+(Cvamb/(Kmv*(1+Cvamb/Kmv))));
