function [ K_add ] = K_adjustment_Hv( coordinatesMatrix,Vmu,Kmu,Kmv,Cuamb,Cvamb,rq)
    ...
    Ke11 = (1/60)*(3*x1 + x2 + x3)*((Vmu*rq*Cuamb)/((Kmv*(Kmu+Cuamb))*(1+Cvamb/Kmv)^2));
    Ke22 = (1/60)*(x1 + 3*x2 + x3)*(Vmu*rq*Cuamb)/((Kmv*(Kmu+Cuamb))*(1+Cvamb/Kmv)^2);
    Ke33 = (1/60)*(x1 + x2 + 3*x3)*(Vmu*rq*Cuamb)/((Kmv*(Kmu+Cuamb))*(1+Cvamb/Kmv)^2);
