function [ K_add ] = K_adjustment_Hv( Ak,coordinatesMatrix,Vmu,Kmu,Kmv,Cuamb,Cvamb,rq)
    ...
    Ke11 = (Ak/18)*(x1 + x2 + x3)*((Vmu*rq*Cuamb)/((Kmv*(Kmu+Cuamb))*(1+Cvamb/Kmv)^2));
    Ke22 = (Ak/18)*(x1 + x2 + x3)*(Vmu*rq*Cuamb)/((Kmv*(Kmu+Cuamb))*(1+Cvamb/Kmv)^2);
    Ke33 = (Ak/18)*(x1 + x2 + x3)*(Vmu*rq*Cuamb)/((Kmv*(Kmu+Cuamb))*(1+Cvamb/Kmv)^2);
